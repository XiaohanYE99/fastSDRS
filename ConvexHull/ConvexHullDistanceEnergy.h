...
class CCBarrierEnergy : public CCDistanceEnergy<T> {
 public:
  ...
  CCBarrierEnergy(const GJKPolytope<T>& p1,const GJKPolytope<T>& p2,const PFunc& p,T d0,const CollisionGradInfo<T>* grad,T coef,bool implicit=false);
  virtual ~CCBarrierEnergy() {}
  bool update(Vec4T* res);
  Eigen::Matrix<T,4,1> getX() const;
  Eigen::Matrix<T,4,1> getG() const;
  Eigen::Matrix<T,4,4> getH() const;
  virtual bool initialize(Vec4T* res,const ArticulatedBody* body);
  void initialize(const Vec4T& x);
  virtual bool eval(T* E,const ArticulatedBody* body,CollisionGradInfo<T>* grad,std::vector<Mat3X4T>* DNDX,
    std::vector<MatX4T>* HThetaX, Vec* GTheta,MatT* HTheta,Vec4T* x=NULL);
