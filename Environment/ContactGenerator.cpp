...
void ContactGenerator::generateManifolds(T x0,bool useCCD,std::vector<ContactManifold>& manifolds,std::set<ContactManifold>& manifoldsID,
  Mat3XT t,int status) {
  updateBVH(t,x0);
    ...
    while(!ss.empty()) {
      ...
      if(!na._bb.intersect(nb._bb))
        ...
        if(!useCCD)
          ...
          if(!manifoldsID.count(m)){
            manifolds.push_back(m);
            manifoldsID.insert(m);
          }
        }
      } else if(na._cell>=0) {
