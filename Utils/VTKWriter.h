...
//for VTK IO
template <typename T>
void vtkWrite(std::ostream& oss,T val);
...
struct VTKWriter {
 ...
 public:
  ...
  VTKWriter(const std::string& name,const std::string& path,bool binary,const BBoxExact& bb,const Eigen::Matrix<int,3,1>& nrCell,bool center)
    :_os(path.c_str(),binary ? std::ios_base::binary : std::ios_base::out),
     _points(binary ? std::ios_base::binary : std::ios_base::out),
     _cells(binary ? std::ios_base::binary : std::ios_base::out),
     _cellTypes(binary ? std::ios_base::binary : std::ios_base::out),
  ...
  template <typename ITER> VTKWriter& appendPoints(ITER beg,ITER end) {
    ...
    if(_binary) {
      for(; beg != end; ++beg) {
        ...
        unsigned char sz=std::min<unsigned char>(3,(unsigned char)val.size()),d=0;
        for(; d<sz; d++)
